...
docname: draft-zartbot-sr-udp-01
date: {DATE}
category: exp
ipr: trust200902
area: Transport
...
SR over UDP must be present at the head of UDP payload. The basic header
shows as below:
...
+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
~~~
{: #srh-basic-format title="SRoU Basic Header"}

The full header shows as below which extend 3 fields:

1.FlowID: Flow tracing and connection migration.

2.SourceAddress and Port: used for NAT-T.

3.HMAC: used for Header integrity check.
...
~~~
{: #srh-full-format title="SRoU Full Header"}
...
H-Bit:
      1 Bit, request HMAC based Authentication Header.

T-Bit:
      1 Bit, request per-hop based inband telemetry.

S-Bit:
      1 Bit, indicate Source Address and Port exist in SRH.

F-Bit:
      2 Bits, Flow ID Type

| Type | Name        |Flow ID Length(bits)       |
|-----:|:------------|:--------------------------|
|  0x0 | NO Flow     | 0                         |
|  0x1 | Short ID    | 64                        |
|  0x2 | Long ID     | 128                       |
|  0x3 | Variable ID | variable length           |
{: #flow-id-type title="Flow ID type field"}
